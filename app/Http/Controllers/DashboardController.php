...
class DashboardController extends Controller
{
    ...
    private function accountstyletotal(){
        return AccountStyles::count();
    }
    private function locationtotal(){
        return Locations::count();
    }
    private function processtotal(){
        return ProcessDataLogs::count();
    }
    private function processerror(){
        $success=ProcessDataLogs::where('process_data_state','1')->count();
        $error=ProcessDataLogs::where('process_data_state','3')->count();

        if($success+$error==0){
            return 0;
        }

        $percentError=($error/($success+$error))*100;
        return $percentError;
    }

    private function lastmonthlysales($stores){
    ...
    private function lastmonthlyprofit($stores){
        $lastmonthlyprofit=DB::table('sale_products')
            ->join('sales','sale_products.sale_no','=','sales.sale_no')
            ->select(DB::raw('SUM(((sale_product_price-sale_product_buy_price)*sale_products.sale_product_qty)-sale_product_disc) as salemargin'))
            ->where(DB::raw('DATE_FORMAT(sales.sale_date,"%m%y")'),'=',$this->lastmonth())
            ->where(DB::raw('LEFT(sales.store_id,4)'),'=',$this->compid())
            ->where('sales.sale_state','!=','0')
            ->first();
    ...
    private function monthlysales($stores){
        $monthlysales=DB::table('sale_products')
            ->join('sales','sale_products.sale_no','=','sales.sale_no')
            ->select(DB::raw('SUM((sale_product_price*sale_product_qty)-sale_product_disc) as saletot'))
    ...
    private function monthlyprofit($stores){
    ...
    private function monthlysalescount($stores){
        $monthsalesqty=Sales::whereMonth('sale_date',Carbon::now()->month)   
            ->whereHas('stores',$stores)->where('sale_state','!=',0)->count('id');
    ...
    private function lastyearlysales($stores){
        $lastyearlysales=DB::table('sales')
            ...
            ->where('stores.comp_id',$this->compid())
            ->where('sales.sale_state','!=','0')
            ->first();
        return $lastyearlysales->saletot;
    }
    private function lastyearlyprofit($stores){
        $lastyearlyprofit=DB::table('sales')
            ->join('sale_products','sales.id','=','sale_products.sale_id')
            ->join('stores','sales.store_id','=','stores.id')
            ->select(DB::raw('SUM(((sale_product_price-sale_product_buy_price)*sale_products.sale_product_qty)-sale_product_disc) as salemargin'))
            ->whereBetween('sales.sale_date',[$this->firstdaylastyear(), $this->samedaylastyear()])
    ...
    private function yearlysales($stores){
    ...
    private function yearlyprofit($stores){
        $yearlyprofit=DB::table('sales')
            ->join('sale_products','sales.id','=','sale_products.sale_id')
            ->join('stores','sales.store_id','=','stores.id')
            ->select(DB::raw('SUM(((sale_product_price-sale_product_buy_price)*sale_products.sale_product_qty)-sale_product_disc) as salemargin'))
            ->where(DB::raw('DATE_FORMAT(sales.sale_date,"%y")'),'=',$this->activeyear())
    ...
    private function yearlysalescount($stores){
        $yearsalesqty=Sales::whereYear('sale_date',Carbon::now()->year)   
            ->whereHas('store',$stores)->where('sale_state','!=',0)->count('id');
        return $yearsalesqty;
    }
    private function lastyearlysalescount($stores){
    ...
    public function index()
    {
        ...
        //             $prodbuyprice=$prod['sale_products']['sale_product_buy_price'];
        //             $prodprice=$prod['sale_products']['sale_product_price'];
        //             $proddisc=$prod['sale_products']['sale_product_disc'];
        //             $prodsaleqty=$prod['sale_products']['sale_product_qty'];
        ...
        //     DB::raw('SUM(((sale_product_price-sale_product_buy_price)*sale_products.sale_product_qty)-sale_product_disc) as salemargin'))
        //     ->where(DB::raw('DATE_FORMAT(sales.sale_date,"%y")'),'=',$this->activeyear())
        //     ->where('stores.comp_id',$this->compid())
        //     ->where('sales.sale_state','!=','0')
        //     ->groupby('salemonth')
        ...
        // if(!$todaysales){
            ...
            $dashboard->dash_process_total=$this->processtotal();
            $dashboard->dash_process_error=$this->processerror();
        ...
        //     $dashboard->dash_revenue_month=$this->yearlysales($stores); 
        //     $dashboard->dash_profit_month=$this->yearlyprofit($stores);
        //     $dashboard->dash_cost_month=$this->yearlysales($stores)-$this->yearlyprofit($stores);
        //     $dashboard->dash_cust_month=$this->yearlysalescount($stores);
        //     $dashboard->dash_revenue_last=$this->lastyearlysales($stores);
        //     $dashboard->dash_profit_last=$this->lastyearlyprofit($stores);
        //     $dashboard->dash_cost_last=$this->lastyearlysales($stores)-$this->lastyearlyprofit($stores);
        //     $dashboard->dash_cust_last=$this->lastyearlysalescount($stores);
        ...
        return view('home/agile/dashboard/dashboardindex',compact("dashboard","activeyear"));
    }
    ...
    public function dashboardStoreMain(Request $request){
        ...
        if(!$sales){
            ...
            foreach ($sales as $key => $sale) {
                foreach ($sale->products as $prod) {
                    $prodbuyprice=$prod['saleproducts']['sale_product_buy_price'];
