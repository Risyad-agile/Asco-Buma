        ...
        return $this->db->table('data_toc as toc')
            ...
            ->map(function ($row) { 
                return [
                    ...
                    $row->T_AmountSpendingStudent ?? 0,

                    $row->T_Teacher ?? 0,
                    $row->T_TeacherOutcome ?? 0,
                    $row->T_TeacherAdditional ?? 0,
                    $row->T_AmountSpendingTeacher ?? 0,

                    $row->T_JobSeeker ?? 0,
                    $row->T_JobSeekerOutcome ?? 0,
                    $row->T_JobSeekerAdditional ?? 0,
                    $row->T_AmountSpendingJobSeeker ?? 0,

                    $row->T_LocalVendor ?? 0,
                    $row->T_LocalVendorOutcome ?? 0,
                    $row->T_LocalVendorAdditional ?? 0,
                    $row->T_AmountSpendingLocalVendor ?? 0,

                    $row->T_Entrepreneur ?? 0,
                    $row->T_EntrepreneurOutcome ?? 0,
                    $row->T_EntrepreneurAdditional ?? 0,
                    $row->T_AmountSpendingLocalEntrepreneur ?? 0,

                    $row->T_Farmer ?? 0,
                    $row->T_FarmerOutcome ?? 0,
                    $row->T_FarmerAdditional ?? 0,
                    $row->T_AmountSpendingLocalFarmer ?? 0,

                    $row->T_BumaEmployee ?? 0,
                    $row->T_BumaEmployeeOutcome ?? 0,
                    $row->T_BumaEmployeeAdditional ?? 0,
                    $row->T_AmountSpendingBumaEmployee ?? 0,

                    !empty($row->created_at) ? substr($row->created_at, 0, 4) : '',
                ];
