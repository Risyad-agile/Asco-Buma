...
@section('content')
<div class="long-title"><h3>Perubahan Status Non Stok</h3></div>
    <div id="tabProd"></div>
</div>
@endsection
...
$(function(){
  ...
  var gridDataSource = new DevExpress.data.DataSource({
    load: function (key) {
        return $.ajax({
            url: "{{route('products.company.state.stock.load')}}"
        })
    },
    update: function (key, activestate) {
          var productid= key.id;
          $.ajax({
              url: "{{URL::to('farma/products/company/state/stock/update')}}"+"/"+productid,
              method: "PUT",
